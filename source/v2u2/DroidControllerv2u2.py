...
# Parameters: Current funds (integer) or request type (string), prompt for input (string), extra value (integer or list of integers)
    ...
    while True:
        ...
        if valid:
            ...
            if playerInput > 0:
                ...
                elif funds == "players" and (playerInput < 2 or playerInput > value):
                    print("Invalid Number: Please provide an integer\n\tfrom 2 up to " + str(value) + ".")
                # Reject nonexistent unordered player number
                elif (funds == "order" and playerInput not in value) or (funds == "winner" and playerInput > value):
                    print("Invalid Player Number: Please enter a valid player number.")
...
# Parameters: Most recently ordered player (string), remaining unordered player names (list of strings), remaining unordered player numbers (list of integers)
# Returns: Next player in physical order (integer)
def RequestOrder(player, playerList, numberOrder):
    # Display either the most recently ordered player or the dealer's player name within prompt
...
# Parameters: Current highest total bet (integer), current player funds (integer)
...
# Parameters: List of potential winners (string), number of potential winners (integer)
...
# Parameters: Opposing player name (string), betting action (string), current highest bet (integer)
# Returns: None
